...
class QuestionSerializer(serializers.ModelSerializer):
    ...
    def get_answers(self, obj):
        if obj.answers.all().exists() and self.context.get('is_detail') == True:
...
class QuizSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Quiz
        fields = ("id", "title", "category_id", "questions")

    def get_questions(self, obj):
        if obj.questions.all().exists() and self.context.get("is_detail") == True:
...
class CategorySerializer(serializers.ModelSerializer):
    ...
    def get_quizzes(self, obj):
        if obj.quizzes.all().exists() and self.context.get("is_detail") == True:
